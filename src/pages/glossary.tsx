...
export class Glossary extends Component {
  private viewTopRow() {
    return (
      <div class="chip-top-row Chip z-20">
        ...
        <div class="w-4/24 sm:w-3/24 px-0 select-none">ABBR</div>
        <div class="w-5/24 sm:w-4/24 px-0 select-none">KIND</div>
      </div>
    );
  }
  ...
  private renderSkills() {
    ...
    return Object.keys(Skills).map((skill) => (
      <div
        class="select-none chip-row Chip"
        data-name={skill}
        onMouseOver={skillMouseoverHandler}
      >
  ...
  private renderStatuses() {
    return Object.keys(Statuses).map((status) => (
      <div
        class="select-none chip-row Mega"
        data-name={status}
        onMouseOver={statusMouseoverHandler}
      >
        <div class="w-8/24 sm:w-6/24 px-0 mx-0 whitespace-nowrap select-none">
  ...
  private renderBlights() {
    ...
    return Object.keys(Blights).map((blight) => (
      <div
        class="select-none chip-row Giga"
        data-name={blight}
        onMouseOver={blightMouseoverHandler}
      >
        <div class="w-8/24 sm:w-6/24 px-0 mx-0 whitespace-nowrap select-none">
  ...
  private renderPanels() {
    return Object.keys(Panels).map((panel) => (
      <div
        class="select-none chip-row Chip"
        data-name={panel}
        onMouseOver={terrainMouseoverHandler}
      >
        <div class="w-8/24 sm:w-6/24 px-0 mx-0 whitespace-nowrap select-none">
  ...
  private renderChipTypes() {
    return Object.keys(ChipTypes).map((chipType) => {
      ...
      return (
        <div
          class={"select-none chip-row " + typeCss}
          data-name={chipType}
          onMouseOver={chipTypeMouseoverHandler}
        >
          <div class="w-8/24 sm:w-6/24 px-0 mx-0 whitespace-nowrap select-none">
