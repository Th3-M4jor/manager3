...
export function sortByName(
  a: FolderChipWithBChip,
  b: FolderChipWithBChip,
): number {
...
export function folderTopRow() {
  return (
    <div class="chip-top-row Chip z-20">
      ...
      <div class="w-3/24 sm:w-3/24 px-0 whitespace-nowrap select-none">DMG</div>
...
export class Folder extends Component<Record<string, never>, FolderState> {
      ...
      <div class="modal">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <input
              type="checkbox"
              id="spectator_checkbox"
              ref={this.spectatorRef}
            />
          </div>
          <div class="modal-footer">
            <span class="pl-1">
              <button class="ok-button" onClick={okCallback}>
                Join
              </button>
            </span>
            <span class="float-right">
              <button class="inactiveNavTab" onClick={cancelCallback}>
                Cancel
              </button>
            </span>
          </div>
  ...
  private getSortedChips(): FolderChipWithBChip[] {
    const folder: FolderChipWithBChip[] = ChipLibrary.ActiveFolder.map(
      ([name, used], idx) => ({
  ...
  private renderChips() {
    if (!ChipLibrary.ActiveFolder.length) {
      return (
        <span class="select-none Chip">{ChipLibrary.FolderName} is Empty!</span>
      );
    }
      ...
      <DropMenu class="dropbtn">
        ...
        <button
          ...
          onClick={() => {
            ...
            top.setTopMsg(
              `${len} ${
                len == 1 ? "chip has" : "chips have"
              } been returned to your pack`,
            );
          }}
        >
          CLEAR FOLDER
    ...
    return (
      <>
        ...
        <div class="col-span-1 flex flex-col px-0 max-h-full">
          ...
          {this.dropMenu()}
          <sort.SortBox
            currentMethod={Folder.sortMethod}
            onSortChange={(e) => {
              Folder.sortMethod = sort.SortOptFromStr(
