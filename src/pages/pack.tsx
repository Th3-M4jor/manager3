...
function sortByOwned(a: PackChipWithBChip, b: PackChipWithBChip): number {
  return (
    cmpN(b.owned.value, a.owned.value) ||
    sort.sortBattleChipByName(a.chip, b.chip)
  );
}

function sortByOwnedDesc(a: PackChipWithBChip, b: PackChipWithBChip): number {
  return (
    cmpN(a.owned.value, b.owned.value) ||
    sort.sortBattleChipByName(a.chip, b.chip)
  );
}
...
async function loadFile(e: Event) {
  ...
  const msg =
    "Are you sure you want to load this file? All current data will be lost";
...
export class Pack extends Component<Record<string, never>, PackState> {
  private static sortMethod: Signal<sort.SortOption> = signal(
  ...
  constructor() {
    ...
    this.addToFolderHandler = (e: Event) => {
      ...
      if (name) {
        top.setTopMsg(
          `A copy of ${name} has been added to ${ChipLibrary.FolderName}`,
        );
      }
      this.forceUpdate();
    };
  ...
  private getSortedChips(): PackChipWithBChip[] {
    ...
    const sortFunc: (a: PackChipWithBChip, b: PackChipWithBChip) => number =
      Pack.sortMethod.value.match({
        AverageDamage: () =>
          Pack.sortDescending.value ? sortByAvgDmgDesc : sortByAvgDmg,
        Element: () =>
          Pack.sortDescending.value ? sortByElemDesc : sortByElem,
        Kind: () => (Pack.sortDescending.value ? sortByKindDesc : sortByKind),
        MaxDamage: () =>
          Pack.sortDescending.value ? sortByMaxDmgDesc : sortByMaxDmg,
        Name: () => (Pack.sortDescending.value ? sortByNameDesc : sortByName),
        Owned: () =>
          Pack.sortDescending.value ? sortByOwnedDesc : sortByOwned,
        Range: () =>
          Pack.sortDescending.value ? sortByRangeDesc : sortByRange,
        Skill: () =>
          Pack.sortDescending.value ? sortBySkillDesc : sortBySkill,
  ...
  private viewTopRow() {
    return (
      <div class="chip-top-row Chip z-20">
        ...
        <div class="w-4/24 sm:w-3/24 px-0 select-none">SKILL</div>
        ...
        <div class="w-2/24 sm:w-2/24 px-0 whitespace-nowrap select-none">O</div>
        <div class="w-2/24 sm:w-2/24 px-0 whitespace-nowrap select-none">U</div>
      </div>
    );
  }
  ...
  private renderChips() {
    if (!ChipLibrary.PackLength) {
      return <span class="select-none Chip">Your Pack is Empty!</span>;
    }

    const chips = this.getSortedChips();

    return chips.map((packChip) => (
      <PackChip
        chip={packChip.chip}
    ...
    return (
      <>
        ...
        <div class="col-span-1 flex flex-col px-0 max-h-full">
          ...
          {this.dropMenu()}
          <sort.SortBox
            currentMethod={Pack.sortMethod.value}
            includeOwned
            onSortChange={(e) => {
              Pack.sortMethod.value = sort.SortOptFromStr(
                (e.target as HTMLSelectElement).value,
              );
              (e.target as HTMLSelectElement).blur(); //unfocus element automatically after changing sort method
            }}
            descending={Pack.sortDescending.value}
            onDescendingChange={(e) => {
              Pack.sortDescending.value = (
    ...
    return (
      <div class="menu" style={style}>
        <ul class="menu-options">
          <li
            class="menu-option select-none"
            onClick={() => {
              if (this.state.contextMenuSelectedId != null) {
                ChipLibrary.removeChipFromPack(
                  this.state.contextMenuSelectedId,
                );
                const name = ChipLibrary.getChip(
                  this.state.contextMenuSelectedId,
                ).name;
