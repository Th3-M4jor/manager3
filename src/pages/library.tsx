...
export class Library extends Component<Record<string, never>, LibraryState> {
  ...
  constructor() {
    ...
    this.doubleClickHandler = (e: Event) => {
      ...
      top.setTopMsg(
        `You now own ${count} ${count == 1 ? "copy" : "copies"} of ${name}`,
      );
    };
  ...
  private sortChips() {
    const sortFunc: (a: BattleChip, b: BattleChip) => number =
      Library.librarySortMethod.match({
        AverageDamage: () =>
          Library.librarySortDescending
            ? sort.sortBattleChipByAvgDmgDesc
            : sort.sortBattleChipByAvgDmg,
        Element: () =>
          Library.librarySortDescending
            ? sort.sortBattleChipByElementDesc
            : sort.sortBattleChipByElement,
        Kind: () =>
          Library.librarySortDescending
            ? sort.sortBattleChipByKindDesc
            : sort.sortBattleChipByKind,
        MaxDamage: () =>
          Library.librarySortDescending
            ? sort.sortBattleChipByMaxDmgDesc
            : sort.sortBattleChipByMaxDmg,
        Name: () =>
          Library.librarySortDescending
            ? sort.sortBattleChipByNameDesc
            : sort.sortBattleChipByName,
        Owned: () => {
          throw new TypeError("Invalid sort method");
        },
        Range: () =>
          Library.librarySortDescending
            ? sort.sortBattleChipByRangeDesc
            : sort.sortBattleChipByRange,
        Skill: () =>
          Library.librarySortDescending
            ? sort.sortBattleChipBySkillDesc
            : sort.sortBattleChipBySkill,
        Cr: () =>
          Library.librarySortDescending
            ? sort.sortBattleChipByCrDesc
            : sort.sortBattleChipByCr,
      });
  ...
  private buildSearchBox() {
    return (
      <>
        <label for="chip_search_box" class="Chip select-none">
          Search
        </label>
        <input
          type="text"
          placeholder="ChipName"
          id="chip_search_box"
  ...
  private renderChips() {
    ...
    if (!chips.length) {
      return <span class="select-none Chip">Nothing matched your search</span>;
    }
    ...
    return (
      <>
        ...
        <div class="col-span-1 flex flex-col px-0 max-h-full">
          <ChipDesc />
          <sort.SortBox
            currentMethod={Library.librarySortMethod}
            onSortChange={(e) => {
              Library.librarySortMethod = sort.SortOptFromStr(
                (e.target as HTMLSelectElement).value,
              );
              this.sortChips();
              (e.target as HTMLSelectElement).blur(); //unfocus element automatically after changing sort method
            }}
            descending={Library.librarySortDescending}
