...
function noGroupTabs(activeTab: top.TabName) {
  const [
    [fldrClass, fldrCallback],
    [packClass, packCallback],
    [libClass, libCallback],
    [glossaryClass, glossaryCallback],
  ] = activeTab.match(baseNavTabMatcher);
  return (
    <>
      <div class="col-span-3 sm:col-span-4 md:col-span-5 pl-2 pr-6 nav-tab-group">
        <button onClick={fldrCallback} class={fldrClass}>
          {ChipLibrary.FolderName}
        </button>
        <button onClick={packCallback} class={packClass}>
          Pack
        </button>
        <button onClick={libCallback} class={libClass}>
          Library
        </button>
      </div>
      <div class="col-span-1 glossary-tab-group">
        <button onClick={glossaryCallback} class={glossaryClass + " w-19/24"}>
          Glossary
        </button>
      </div>
    </>
...
function withGroupTabs(activeTab: top.TabName) {
  ...
  const buttons = folders.map((f) => {
    ...
    const shortName =
      ...
      name.trim().slice(1, maxNameLen - 1);

    const [cssClass, callback]: ClassAndCallback = activeTab.match({
      GroupFolder: (playerName) =>
        playerName == name
          ? (["activeNavTab", dontRedraw] as ClassAndCallback)
          : ([
              "inactiveNavTab",
              makeChangeToPlayerFn(name),
            ] as ClassAndCallback),
      _: () =>
        ["inactiveNavTab", makeChangeToPlayerFn(name)] as ClassAndCallback,
    });
...
export function NavTabs() {
  ...
  return folders?.length
    ? withGroupTabs(top.getActiveTab())
    : noGroupTabs(top.getActiveTab());
}
