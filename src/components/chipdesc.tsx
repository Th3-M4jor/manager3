...
export const ChipDescDisplay = makeTaggedUnion({
  None: none,
  ChipId: (id: number) => id,
  GlossaryItem: (name: string, backgroundCss: string, text: string) => ({
    name,
    backgroundCss,
    text,
  }),
...
function fadeIn(e: AnimationEvent) {
  ...
  (e.target as HTMLDivElement).classList.replace(
    "chipDescFadeOut",
    "chipDescFadeIn",
  );
...
export class ChipDesc extends Component {
  ...
  private elemRow(chip: BattleChip) {
    return (
      <>
        <div class="chipDescLeft chipBasicInfo">elem:</div>
        <div class="chipDescRight chipBasicInfo">{chip.renderElements()}</div>
      </>
    );
  }
  ...
  private classRow(chip: BattleChip) {
    return ["Standard", "Support"].includes(chip.class.variant) ? null : (
      <>
        <div class="chipDescLeft chipBasicInfo">class:</div>
        <div class="chipDescRight chipBasicInfo">{chip.class.variant}</div>
      </>
    );
  }

  private crRow(chip: BattleChip) {
    return ["Standard", "Support"].includes(chip.class.variant) &&
      chip.cr > 0 ? (
      <>
        <div class="chipDescLeft chipBasicInfo">cr:</div>
        <div class="chipDescRight chipBasicInfo">{chip.cr}</div>
      </>
    ) : null;
  }
  ...
  private dmgRow(chip: BattleChip) {
    return chip.damage ? (
      <>
        <div class="chipDescLeft chipBasicInfo">dmg:</div>
        <div class="chipDescRight chipBasicInfo">{chip.dmgStr}</div>
      </>
    ) : null;
  }
  ...
  private kindRow(chip: BattleChip) {
    return (
      <>
        <div class="chipDescLeft chipBasicInfo">kind:</div>
        <div class="chipDescRight chipBasicInfo">{chip.KindAbv}</div>
      </>
    );
  }
  ...
  private skillRow(chip: BattleChip) {
    return chip.Skill.variant != "None" ? (
      <>
        <div class="chipDescLeft chipBasicInfo">skill:</div>
        <div class="chipDescRight chipBasicInfo">{chip.SkillAbv}</div>
      </>
    ) : null;
  }
  ...
  private rangeRow(chip: BattleChip) {
    return (
      <>
        <div class="chipDescLeft chipBasicInfo">range:</div>
        <div class="chipDescRight chipBasicInfo">{chip.RangeAbv}</div>
      </>
    );
  }
  ...
  private hitsRow(chip: BattleChip) {
    return chip.hits !== "0" ? (
      <>
        <div class="chipDescLeft chipBasicInfo">hits:</div>
        <div class="chipDescRight chipBasicInfo">{chip.hits}</div>
      </>
    ) : null;
  }
  ...
  private targetsRow(chip: BattleChip) {
    return chip.targets && chip.targets !== "0" ? (
      <>
        <div class="chipDescLeft chipBasicInfo">trgts:</div>
        <div class="chipDescRight chipBasicInfo">{chip.targets}</div>
      </>
    ) : null;
  }
  ...
  private blightRows(chip: BattleChip) {
    return chip.blight ? (
      <>
        <div class="chipDescLeft chipBasicInfo">blight:</div>
        <div class="chipDescRight chipBasicInfo">
          <span class="chipImgBox">
            <span class={elementToCssClass(chip.blight.elem)} />
          </span>
        </div>
        <div class="chipDescLeft chipBasicInfo">bdmg:</div>
        <div class="chipDescRight chipBasicInfo">
          {diceToStr(chip.blight.dmg)}
        </div>
        <div class="chipDescLeft chipBasicInfo">bdur:</div>
        <div class="chipDescRight chipBasicInfo">
  ...
  private viewGlossary(
    ...
    const chipAnimClass =
      this.animationCounter & 1 ? "chipWindowOne" : "chipWindowTwo";

    const outerChipClass =
      "chipDescText chipDescPadding max-h-full flex flex-col " + chipAnimClass;

    return (
      <div
        class={background}
        style="max-height: 85vh"
        onMouseEnter={descMouseOverHandler}
        onMouseLeave={descMouseLeaveHandler}
      >
        <div class={outerChipClass} style="padding: 3px; height: 100%">
          <div class="border-b border-black chipName">{name}</div>
          <div class="grid grid-cols-2 gap-0"></div>
          <div
            class="overflow-y-scroll hideScrollBar m-0"
            style={fontSizeStyle}
            id="ScrollTextDiv"
          >
            {description}
  ...
  render() {
    return activeDisplayItem.value.match({
      ...
      GlossaryItem: ({ name, backgroundCss, text }) =>
        this.viewGlossary(name, backgroundCss, text),
    });
  }
}
